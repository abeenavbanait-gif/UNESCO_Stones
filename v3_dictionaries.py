# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  V3 MEGA-DICTIONARY MODULE (GEOLOGICAL & ARCHITECTURAL HERITAGE)            ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# Flatten named_stones and name_keywords
all_named_stones = []
for k, v in named_stones.items():
    all_named_stones.extend(v)

all_arch_categories = []
for k, v in name_keywords.items():
    all_arch_categories.extend(v)

# Foundational Basic Terms added specifically for V3
foundational_geo_terms = [
    ...
    'bedrock', 'outcrop', 'mineral', 'minerals', 'crystal', 'crystals', 'crystalline',
...
additional_cult_terms = [
    ...
    'building', 'buildings', 'built', 'ruins', 'ruined', 'settlement', 'city', 'town', 'village',
    'inscription', 'inscriptions', 'petroglyph', 'engraving', 'cladding', 'veneer', 'facing', 'revetment'
]

# Combined Master V3 Lists for spaCy Tagging
GEO_MATERIALS = sorted(list(set(
    foundational_geo_terms + 
    igneous_rocks + 
    sedimentary_rocks + 
    metamorphic_rocks + 
    all_named_stones + 
    decorative_minerals + 
    [m.lower() for m in building_materials if any(k in m.lower() for k in ['stone', 'rock', 'marble', 'granite', 'slate', 'limestone', 'sandstone', 'basalt', 'tuff'])]
)))

CULTURAL_HERITAGE = sorted(list(set(
    all_arch_categories + 
    construction_terms + 
    architecture_terms + 
    construction_verbs + 
...
if __name__ == '__main__':
    print(f'✅ Extracted {len(GEO_MATERIALS)} Geological Materials and {len(CULTURAL_HERITAGE)} Cultural Heritage Terms for V3!')
